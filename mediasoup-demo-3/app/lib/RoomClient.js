...
export default class RoomClient {
  ...
  constructor({
    ...
    // 전송을 위한 H264 코덱을 강제 실행 여부
    this._forceH264 = Boolean(forceH264);

    // 전송을 위해 VP9 코덱을 강제 실행 여부
    ...
    // 사용자 정의 mediasoup-client 핸들러 이름(원하는 경우 기본 브라우저 감지를 재정의하기 위함).
    // @type {String}
    ...
    // @see https://www.npmjs.com/package/protoo-client
    // protoo는 다자간 실시간 통신 애플리케이션을 위한 미니멀하고 확장 가능한 Node.js 신호 프레임워크입니다.
    ...
    // 로컬 mic mediasoup Producer.
    // @type {mediasoupClient.Producer}
    this._micProducer = null;

    // 로컬 webcam mediasoup Producer.
    // @type {mediasoupClient.Producer}
    this._webcamProducer = null;

    // 로컬 share mediasoup Producer.
    // @type {mediasoupClient.Producer}
    this._shareProducer = null;

    // 로컬 chat DataProducer.
    ...
    // deviceId로 인덱싱된 웹캠 MediaDeviceInfos의 Map
    ...
    // 사용자 지정 SVC scalability 모드를 설정한다.
    if (svc) {
      ...
      switch (request.method) {
        case "newConsumer": {
          ...
          try {
            ...
            // 준비 됨.
            // protoo 요청에 응답하여 서버가 이 consumer를 재개한다.(비디오인 경우 지금은 일시 중지됨).
            accept();

            // 오디오 전용 모드가 활성화된 경우 일시 중지한다.
  ...
  async enableWebcam() {
    ...
    try {
      ...
      if (this._useSimulcast) {
        // VP9가 유일하게 사용 가능한 비디오 코덱인 경우 SVC를 사용하십시오.
  ...
  async changeWebcam() {
    ...
    try {
      ...
      // 새 트랙을 요청하기 전에 현재 비디오 트랙을 닫습니다
      // (모바일은 전면/후면 카메라를 동시에 여는 것을 좋아하지 않습니다).
  ...
  async enableChatDataProducer() {
    ...
    // NOTE: 이 코드를 활성화해야 하지만 테스트에 유용합니다.
  ...
  async changeDisplayName(displayName) {
    ...
    try {
      ...
      // 이전 요소를 렌더링하려면 구성 요소를 새로 고쳐야 합니다.
      // 다시 displayName.
  ...
  async _joinRoom() {
    ...
    try {
      ...
      // NOTE: 브라우저의 새로운 자동 재생 정책으로 인해 원격 오디오를 재생하는 항목입니다.
      //
      // 마이크에 액세스하고 잠시 동안 마이크 트랙을 닫지 마십시오.
      // Super hack!
      {
        const stream = await navigator.mediaDevices.getUserMedia({
          audio: true,
      ...
      // 전송을 위해 mediasoup Transport를 생성합니다(생성하지 않으려는 경우 제외).
      if (this._produce) {
        ...
        this._sendTransport = this._mediasoupDevice.createSendTransport({
          ...
          dtlsParameters: {
            ...dtlsParameters,
            // 원격 DTLS 역할. 기본적으로 항상 '자동'이라는 것을 알고 있으므로
            // 우리는 로컬 WebRTC 전송이 '클라이언트'가 되도록 강제할 수 있습니다.
            // 여기서 '서버'를 나타내고 그 반대도 마찬가지입니다.
            role: "auto",
          },
          sctpParameters,
          iceServers: [],
      ...
      // 수신을 위해 mediasoup Transport를 생성합니다(consume를 원하지 않는 경우).
      if (this._consume) {
        const transportInfo = await this._protoo.request(
          "createWebRtcTransport",
          {
      ...
      // 지금 방에 참여
      // NOTE: 사용하지 않으려면 RTP 기능을 보내지 마십시오.
